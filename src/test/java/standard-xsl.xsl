...
<xsl:stylesheet version="1.0"
	...
	<!-- <xsl:variable name="mediaPath">http://localhost:8080/cvconverter/pdf-media</xsl:variable> -->
		...
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
			...
			<fo:page-sequence master-reference="new-cover"
				...
				<fo:static-content flow-name="new-cover-head">
					...
					<fo:block-container absolute-position="absolute" top="210mm" left="20mm">
						...
						<fo:block>
						<!-- Base64 encoded file: sogeti logo -->
						<fo:external-graphic content-height="scale-down-to-fit"
							content-width="scale-down-to-fit"
							src="url('data:image/png;base64, ')" />
						</fo:block>						
					</fo:block-container>
				...
				<fo:flow flow-name="xsl-region-body">
					<xsl:if test="cv:Tags">
						<fo:block font-size="30px" font-family="sans-serif" color="rgb(242,101,50)">
							"
						</fo:block>
					</xsl:if>
			...
			<fo:page-sequence master-reference="cover"
				...
				<fo:flow flow-name="xsl-region-body">
						...
						<fo:table-body>
							<fo:table-row>
								<fo:table-cell>
									...
									<xsl:apply-templates select="cv:Profile/cv:DateOfBirth" />																		
					...
					<!-- <xsl:call-template name="importantProfessionalKnowledge" /> -->
			...
			<fo:page-sequence master-reference="sequence"
				...
				<fo:flow flow-name="xsl-region-body">
					...
					<xsl:apply-templates select="cv:ProfessionalKnowledge" />

					<!-- <xsl:apply-templates select="cv:Technology" /> -->
					<xsl:apply-templates select="cv:IndustryKnowledge" />
					<!-- <xsl:apply-templates select="cv:Certifications" /> <xsl:apply-templates 
						select="cv:ForeignLanguages" /> -->
					<xsl:apply-templates select="cv:Courses" />
					<!-- <xsl:apply-templates select="cv:Organisations" /> -->
			...
			<fo:table table-layout="fixed" width="100%">
				<fo:table-body>
					<fo:table-row height="9mm">
						...
						<fo:table-cell text-align="center" display-align="center">
							<fo:block>072-728 78 26</fo:block>
						</fo:table-cell>
						<fo:table-cell text-align="center" display-align="center">
							<fo:block>www.sogeti.se</fo:block>
						</fo:table-cell>
						<fo:table-cell text-align="center" display-align="center">
							<fo:block>130625</fo:block>
						</fo:table-cell>
						<fo:table-cell text-align="center" display-align="center">
							<fo:block>
								<fo:page-number />
	...
	<xsl:template name="name">
		<fo:block font-weight="bold" font-size="14px">
			<xsl:value-of select="cv:Profile/cv:Name" />
		</fo:block>
	</xsl:template>

	<xsl:template match="cv:DateOfBirth">
		<fo:block>
	...
	<xsl:template name="importantTechnologies">
		<xsl:if test="cv:Technology/cv:Skill[@important='true']">
			...
			<fo:list-block>
				<xsl:for-each select="cv:Technology/cv:Skill[@important='true'][position() &lt; 11]">
					<fo:list-item>
						<fo:list-item-label start-indent="1mm">
							<fo:block>&#8226;</fo:block>
						</fo:list-item-label>
	...
	<xsl:template match="cv:ProfessionalKnowledge">
		<fo:block font-size="{$headlineFontSize}" font-weight="bold"
			margin-bottom="3mm" margin-left="30mm">
			PROFESSIONAL KNOWLEDGE
		</fo:block>
		<fo:list-block>
			<xsl:apply-templates select="cv:Skill" />
		</fo:list-block>
	</xsl:template>

	<xsl:template match="cv:IndustryKnowledge">
		<fo:block font-size="{$headlineFontSize}" font-weight="bold"
			margin-top="3mm" margin-bottom="3mm" margin-left="30mm">  <!-- check margin -->
			INDUSTRY KNOWLEDGE
		</fo:block>
		<fo:list-block>
			<xsl:apply-templates select="cv:Skill" />
	...
	<xsl:template match="cv:Courses">
		...
		<fo:list-block>
			<xsl:apply-templates select="cv:Acquisition" />
		</fo:list-block>
	</xsl:template>

	<xsl:template match="cv:Employments">
		<fo:block font-size="{$headlineFontSize}" font-weight="bold"
			margin-top="3mm" margin-bottom="3mm" margin-left="30mm"> <!-- check margin -->
			<xsl:choose>
				<xsl:when test="//@language = 'en'">
	...
	<xsl:template match="cv:Skill|cv:Language">
		<fo:list-item>
			<fo:list-item-label start-indent="1mm">
				<fo:block>&#8226;</fo:block>
			</fo:list-item-label>
	...
	<xsl:template match="cv:Acquisition">
		<fo:list-item border-bottom="{$border}">
			<fo:list-item-label>
				<fo:block>
					<xsl:value-of select="cv:Date" />
